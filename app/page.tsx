...
export default function Home() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        setError(
          `Database connection error: ${err.message}. Please check your environment variables.`
        );
      }
    };
  ...
  const createSession = async () => {
    ...
    try {
      ...
      router.push(
        `/session/${newSessionId}?user=${encodeURIComponent(
          userName
        )}&userId=${userId}`
      );
    } catch (err) {
  ...
  const joinSession = async () => {
    ...
    try {
      ...
      router.push(
        `/session/${sessionId.toUpperCase()}?user=${encodeURIComponent(
          userName
        )}&userId=${userId}`
      );
    } catch (err) {
  ...
  return (
    <div className="container flex items-center justify-center min-h-screen py-8">
      <Card className="w-full max-w-md shadow-lg border-0">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <Button
              ...
              onClick={joinSession}
              disabled={
                isCreating || isJoining || !sessionId.trim() || !userName.trim()
              }
            >
              {isJoining ? (
                <>
          ...
          <p className="text-xs text-muted-foreground text-center">
            Create a session and share the link with your team members to start
            estimating
          </p>
          {error && (
        ...
        <CardFooter className="flex justify-center flex-col border-t pt-6">
          <p className="text-xs text-muted-foreground text-center">
            Made By: Sehajpreet Singh{" "}
          </p>
          <a
            href="https://github.com/sehaj97"
